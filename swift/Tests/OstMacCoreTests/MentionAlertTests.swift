...
final class MentionAlertTests: XCTestCase {
    ...
    func testQuietDisabledOrDegenerateNeverFires() {
        ...
        XCTAssertFalse(QuietHours(enabled: false).isActive(at: noon, calendar: cal))
        // Equal bounds = empty window, even when enabled.
        XCTAssertFalse(QuietHours(enabled: true, startMinutes: 60, endMinutes: 60)
            .isActive(at: dateAt(hour: 1, calendar: cal), calendar: cal))
    }

    func testQuietSameDayWindow() {
        let cal = fixedCalendar()
        let q = QuietHours(enabled: true, startMinutes: 9 * 60, endMinutes: 17 * 60)
        XCTAssertFalse(q.isActive(at: dateAt(hour: 8, minute: 59, calendar: cal), calendar: cal))
        XCTAssertTrue(q.isActive(at: dateAt(hour: 9, calendar: cal), calendar: cal))
        XCTAssertTrue(q.isActive(at: dateAt(hour: 12, calendar: cal), calendar: cal))
        XCTAssertFalse(q.isActive(at: dateAt(hour: 17, calendar: cal), calendar: cal))
    }

    func testQuietOvernightWindow() {
        let cal = fixedCalendar()
        let q = QuietHours(enabled: true, startMinutes: 22 * 60, endMinutes: 7 * 60)
        XCTAssertTrue(q.isActive(at: dateAt(hour: 23, calendar: cal), calendar: cal))
        XCTAssertTrue(q.isActive(at: dateAt(hour: 0, calendar: cal), calendar: cal))
        XCTAssertTrue(q.isActive(at: dateAt(hour: 6, minute: 59, calendar: cal), calendar: cal))
        XCTAssertFalse(q.isActive(at: dateAt(hour: 7, calendar: cal), calendar: cal))
        XCTAssertFalse(q.isActive(at: dateAt(hour: 12, calendar: cal), calendar: cal))
        XCTAssertFalse(q.isActive(at: dateAt(hour: 21, minute: 59, calendar: cal), calendar: cal))
        XCTAssertTrue(q.isActive(at: dateAt(hour: 22, calendar: cal), calendar: cal))
    }

    func testQuietSummaryAndClamp() {
        XCTAssertEqual(QuietHours(enabled: false).summary, "off")
        XCTAssertEqual(
            QuietHours(enabled: true, startMinutes: 22 * 60, endMinutes: 7 * 60).summary,
            "22:00–07:00")
        // Out-of-range bounds clamp into the day.
        let q = QuietHours(enabled: true, startMinutes: -5, endMinutes: 9999)
        XCTAssertEqual(q.startMinutes, 0)
        XCTAssertEqual(q.endMinutes, 1439)
    }

    func testQuietMinutesRoundTrip() {
        let cal = fixedCalendar()
        let at = dateAt(hour: 22, minute: 30, calendar: cal)
        XCTAssertEqual(QuietHours.minutesOfDay(at, calendar: cal), 22 * 60 + 30)
        // Picker dates carry the stored minutes back.
        let back = QuietHours.date(forMinutes: 7 * 60 + 5, calendar: cal)
        XCTAssertEqual(QuietHours.minutesOfDay(back, calendar: cal), 7 * 60 + 5)
    }

    func testQuietStorePersists() {
        let defaults = isolatedDefaults()
        let first = QuietHoursStore(defaults: defaults)
        XCTAssertFalse(first.hours.enabled)
        first.hours = QuietHours(enabled: true, startMinutes: 60, endMinutes: 120)
        ...
        let cal = fixedCalendar()
        XCTAssertTrue(second.isActiveNow(dateAt(hour: 1, calendar: cal), calendar: cal))
        XCTAssertFalse(second.isActiveNow(dateAt(hour: 3, calendar: cal), calendar: cal))
    }
    ...
    func testMentionDiagnosticsLines() {
        ...
        XCTAssertEqual(DiagnosticsFormat.quietLine(summary: "off", active: false), "off")
        XCTAssertEqual(
            DiagnosticsFormat.quietLine(summary: "22:00–07:00", active: false),
            "22:00–07:00")
        XCTAssertEqual(
            DiagnosticsFormat.quietLine(summary: "22:00–07:00", active: true),
            "22:00–07:00 · active")
    }
}

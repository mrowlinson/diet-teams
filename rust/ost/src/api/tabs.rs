...
//! id; the owning team resolves via a joinedTeams scan (files.rs parity).
//! Auth: existing Graph token, no scope widening. No tab content is
//! fetched here — Swift deep-links Files/Notes into its own views and
//! opens web-tab URLs in the browser.
...
/// Swift maps well-known tabs (Posts/Files/Notes) to its own views;
/// anything with a URL opens in the browser. No content renderers.
pub struct TabInfo {
    pub id: String,
    pub name: String,

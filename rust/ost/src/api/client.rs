...
impl TeamsClient {
    ...
    /// OstMac (om-remind): To Do task completion flips `status` via PATCH.
    ...
    /// API; om-reactions: reaction removal; om-editdel: message deletes
    /// pass `None`). Optional JSON body, same auth as [`Self::chat_post`].

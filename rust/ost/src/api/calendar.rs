...
//!
//! OstMac (om-meet-join): the Meetings tab lists the signed-in user's
//! upcoming events with Teams join URLs (Graph `calendarView`), and the
//! join box accepts a pasted Teams link (or raw thread id). Auth reuses
//! the existing Graph token (`https://graph.microsoft.com/.default`);
//! no scope widening was needed: the first-party Teams client id already
//! consents `Calendars.Read`, and a 403 surfaces as the call's detail.
//!
...
//! so the embedder drives `idle → joining → lobby → admitted|failed`
...
// ---------------------------------------------------------------------------
// Embedder-facing meeting record
// ---------------------------------------------------------------------------
...
/// Pure (no network) so embedders and tests pin the shape.
...
/// `unknown` (not joinable — the embedder shows a hint, never dials).

...
//!
//! The `If-Match` PATCH cannot go through [`TeamsClient::graph_patch`]
//! (no header support, and the token getter is crate-private), so
//! [`set_task_complete_data`] builds its own PATCH from the cached
//! config (fresh after [`TeamsClient::new`], which refreshes first).
//! Its status check mirrors `client::check_response`, which is private.
...
/// Status check for the raw `If-Match` PATCH. Mirrors the private
/// `client::check_response` (401 hint + status + body).
async fn check_status(resp: reqwest::Response, url: &str) -> Result<reqwest::Response> {
    ...
    if status == reqwest::StatusCode::PRECONDITION_FAILED {
        bail!(
            "412 Precondition Failed for {}: stale etag -- refresh the board and retry.",
            url
        );
    }
...
/// the board moved under the caller. Call [`TeamsClient::new`] first so
/// the cached config (token source here) is refreshed.
pub async fn set_task_complete_data(
    ...
    let resp = super::client::shared_http()
        ...
        .send()
        .await
        .with_context(|| format!("Planner PATCH {} failed", url))?;
    let resp = check_status(resp, &url).await?;
    let bytes = resp.bytes().await.context("Failed to read updated task body")?;
    parse_task(&bytes)
}

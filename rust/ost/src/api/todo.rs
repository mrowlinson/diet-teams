...
//!
//! OstMac (om-remind): the Reminders tab reads the signed-in user's real
//! Microsoft To Do data — no local-only fallback. Auth reuses the existing
//! Graph token (`https://graph.microsoft.com/.default` + `offline_access`);
//! no scope widening was needed: the first-party Teams client id already
//! consents `Tasks.ReadWrite`, and a 403 surfaces as the call's detail.
